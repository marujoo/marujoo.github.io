...
function addBotoes (produtos) {

    const butao = document.querySelector("#remover-todos");
    butao.addEventListener('click', () => {
        produtos.forEach(produto => removerDoCesto(produto));
    });

    const button = document.querySelector("#remover-descricao");
    button.addEventListener('click', () => {
        const descricoes = document.querySelectorAll(".descricao");
        descricoes.forEach(descricao => descricao.remove());
    });
}
